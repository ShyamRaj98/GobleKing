...
const images = [
    ...
    },
    {
        ...
        imgPath: slide_img_7,
        content: 'Document Control Service encompasses the creation, revision, and distribution of project documents, ranging from design drawings and specifications to project plans and reports.'
    },
];
...
function MobileCardService() {
    ...
    return (
        <div className='mobile_service_card' style={{height: '620px'}}>
            <Slider
                    ...
                    <Link key={i} className='animated_card' to={`/service/${step.link}`}>
                        <div className='card'>
                            ...
                            <div className='card_title'>
                                <h1 style={{borderBottom: '1px solid black',height: 'auto'}}>{step.label}</h1>
                                <p>{step.content}</p>
                            </div>
                        </div>
                    </Link>
...
function ServiceCard() {
    ...
    return (
        <>
            {!isMobile ?
                (
                    <div className='card__service' style={{marginTop:'0px',marginBottom: '50px'}}>
                                ...
                                {images.map((step, index) => (
                                    <Link key={index} className='animated_card' to={`/service/${step.link}`}>
                                        <div className='card'>
                                            ...
                                            <div className='card_title'>
                                                <h1 style={{borderBottom: '1px solid black'}}>{step.label}</h1>
                                                <p>{step.content}</p>
                                            </div>
                                        </div>
                                    </Link>
